...
LanguageProto _create() {
  ...
  const definition_17 =
      '(^[\\t ]*)#\\s*[a-z](?:[^\\r\\n\\\\/]|\\/(?!\\*)|\\/\\*(?:[^*]|\\*(?!\\/))*\\*\\/|\\\\(?:\\r\\n|[\\s\\S]))*';
  const definition_43 = '^(#\\s*include\\s*)<[^>]+>';
  const definition_44 = '"(?:\\\\(?:\\r\\n|[\\s\\S])|[^"\\\\\\r\\n])*"';
  const definition_45 = '(^#\\s*define\\s+)\\w+\\b(?!\\()';
  const definition_46 = '(^#\\s*define\\s+)\\w+\\b(?=\\()';
  const definition_41 = '^(#\\s*)[a-z]+';
  const definition_34 = '^#';
  const definition_35 = '##|\\\\(?=[\\r\\n])';
  const definition_42 = '\\S[\\s\\S]*';
  const definition_20 = '@?"(?:\\\\(?:\\r\\n|[\\s\\S])|[^"\\\\\\r\\n])*"';
  const definition_21 =
      '\\b(?:cl_(?:GLenum|GLint|GLuin|addressing_mode|bitfield|bool|buffer_create_type|build_status|channel_(?:order|type)|(?:u?(?:char|int|long|short)|double|float)(?:2|3|4|8|16)?|command_(?:queue(?:_info|_properties)?|type)|context(?:_info|_properties)?|device_(?:exec_capabilities|fp_config|id|info|local_mem_type|mem_cache_type|type)|(?:event|sampler)(?:_info)?|filter_mode|half|image_info|kernel(?:_info|_work_group_info)?|map_flags|mem(?:_flags|_info|_object_type)?|platform_(?:id|info)|profiling_info|program(?:_build_info|_info)?))\\b';
  const definition_23 = '\\bCL_(?:FALSE|TRUE)\\b';
  const definition_25 =
  ...
  final definition_3 = GrammarToken(
    pattern: RegExp(definition_17, caseSensitive: false, multiLine: true),
    lookbehind: true,
    greedy: true,
    aliases: ['property'],
  ...
  definition_19.addAllTokens({
    ...
    'char': [definition_30],
    'comment': [definition_31],
    'macro-name': [
      definition_39,
      definition_40,
    ],
    'directive': [definition_33],
    'directive-hash': [GrammarToken(pattern: RegExp(definition_34))],
  ...
  definition_0.addAllTokens({
    ...
    'macro': [definition_3],
    'string': [definition_4],
    'type-opencl-host': [definition_5],
    'boolean-opencl-host': [definition_6],
    'constant-opencl-host': [definition_7],
    'function-opencl-host': [definition_8],
...
final languageObjectivec = _create();
