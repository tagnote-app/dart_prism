...
LanguageProto _create() {
  ...
  const definition_18 =
      '(^[\\t ]*)#\\s*[a-z](?:[^\\r\\n\\\\/]|\\/(?!\\*)|\\/\\*(?:[^*]|\\*(?!\\/))*\\*\\/|\\\\(?:\\r\\n|[\\s\\S]))*';
  const definition_42 = '^(#\\s*include\\s*)<[^>]+>';
  const definition_21 = '"(?:\\\\(?:\\r\\n|[\\s\\S])|[^"\\\\\\r\\n])*"';
  const definition_43 = '(^#\\s*define\\s+)\\w+\\b(?!\\()';
  const definition_44 = '(^#\\s*define\\s+)\\w+\\b(?=\\()';
  const definition_40 = '^(#\\s*)[a-z]+';
  const definition_34 = '^#';
  const definition_35 = '##|\\\\(?=[\\r\\n])';
  const definition_41 = '\\S[\\s\\S]*';
  const definition_22 =
