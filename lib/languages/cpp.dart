...
LanguageProto _create() {
  ...
  const definition_25 =
      '(^[\\t ]*)#\\s*[a-z](?:[^\\r\\n\\\\/]|\\/(?!\\*)|\\/\\*(?:[^*]|\\*(?!\\/))*\\*\\/|\\\\(?:\\r\\n|[\\s\\S]))*';
  const definition_77 = '^(#\\s*include\\s*)<[^>]+>';
  const definition_32 = '"(?:\\\\(?:\\r\\n|[\\s\\S])|[^"\\\\\\r\\n])*"';
  const definition_78 = '(^#\\s*define\\s+)\\w+\\b(?!\\()';
  const definition_79 = '(^#\\s*define\\s+)\\w+\\b(?=\\()';
  const definition_72 = '^(#\\s*)[a-z]+';
  const definition_58 = '^#';
  const definition_59 = '##|\\\\(?=[\\r\\n])';
  ...
  const definition_64 = '\\b[a-z_]\\w*\\b(?!\\s*::)';
  const definition_51 = '::';
  const definition_20 =
  ...
  const definition_36 = '\\b[A-Z]\\w*(?=\\s*::\\s*\\w+\\s*\\()';
  const definition_37 = '\\b[A-Z_]\\w*(?=\\s*::\\s*~\\w+\\s*\\()';
  const definition_38 =
      '\\b\\w+(?=\\s*<(?:[^<>]|<(?:[^<>]|<[^<>]*>)*>)*>\\s*::\\s*\\w+\\s*\\()';
  ...
  final definition_3 = GrammarToken(
    pattern: RegExp(definition_25, caseSensitive: false, multiLine: true),
    lookbehind: true,
    greedy: true,
    aliases: ['property'],
  ...
  final definition_18 = GrammarToken(
    pattern: RegExp(definition_50, caseSensitive: false),
    greedy: true,
    global: true,
  );
  ...
  definition_27.addAllTokens({
    ...
    'char': [definition_54],
    'comment': [definition_55],
    'macro-name': [
      definition_70,
      definition_71,
    ],
    'directive': [definition_57],
    'directive-hash': [GrammarToken(pattern: RegExp(definition_58))],
    'punctuation': [GrammarToken(pattern: RegExp(definition_59))],
    'expression': [definition_60],
  });
  definition_29.addAllTokens({
    'string': [GrammarToken(pattern: RegExp(definition_61))],
    'operator': [GrammarToken(pattern: RegExp(definition_62))],
    'punctuation': [GrammarToken(pattern: RegExp(definition_63))],
  });
  definition_40.addAllTokens({
    'function': [GrammarToken(pattern: RegExp(definition_66))],
    'generic': [definition_67],
  });
  definition_34.addAllTokens({
    ...
    'class-name': [
      GrammarToken(pattern: RegExp(definition_64, caseSensitive: false))
    ],
  ...
  return LanguageProto(
    ...
    optionalDependencies: ['opencl-extensions'],
  );
}

final languageCpp = _create();
