...
LanguageProto _create() {
  ...
  const definition_19 =
      '(^[\\t ]*)#\\s*[a-z](?:[^\\r\\n\\\\/]|\\/(?!\\*)|\\/\\*(?:[^*]|\\*(?!\\/))*\\*\\/|\\\\(?:\\r\\n|[\\s\\S]))*';
  const definition_47 = '^(#\\s*include\\s*)<[^>]+>';
  const definition_22 = '"(?:\\\\(?:\\r\\n|[\\s\\S])|[^"\\\\\\r\\n])*"';
  const definition_48 = '(^#\\s*define\\s+)\\w+\\b(?!\\()';
  const definition_49 = '(^#\\s*define\\s+)\\w+\\b(?=\\()';
  const definition_45 = '^(#\\s*)[a-z]+';
  const definition_38 = '^#';
  const definition_39 = '##|\\\\(?=[\\r\\n])';
  const definition_46 = '\\S[\\s\\S]*';
  const definition_23 =
      '(\\b(?:enum|struct)\\s+(?:__attribute__\\s*\\(\\([\\s\\S]*?\\)\\)\\s*)?)\\w+|\\b[a-z]\\w*_t\\b';
  const definition_24 =
      '\\b(?:cl_(?:GLenum|GLint|GLuin|addressing_mode|bitfield|bool|buffer_create_type|build_status|channel_(?:order|type)|(?:u?(?:char|int|long|short)|double|float)(?:2|3|4|8|16)?|command_(?:queue(?:_info|_properties)?|type)|context(?:_info|_properties)?|device_(?:exec_capabilities|fp_config|id|info|local_mem_type|mem_cache_type|type)|(?:event|sampler)(?:_info)?|filter_mode|half|image_info|kernel(?:_info|_work_group_info)?|map_flags|mem(?:_flags|_info|_object_type)?|platform_(?:id|info)|profiling_info|program(?:_build_info|_info)?))\\b';
  const definition_26 = '\\bCL_(?:FALSE|TRUE)\\b';
  const definition_28 =
  ...
  const definition_32 = '\\bcilk_(?:for|reducer|s(?:cope|pawn|ync))\\b';
  ...
  final definition_43 = GrammarToken(
    pattern: RegExp(definition_48, caseSensitive: false),
    lookbehind: true,
  );
  ...
  definition_21.addAllTokens({
    ...
    'directive': [definition_37],
    'directive-hash': [GrammarToken(pattern: RegExp(definition_38))],
    'punctuation': [GrammarToken(pattern: RegExp(definition_39))],
    'expression': [definition_40],
  });
  definition_0.addAllTokens({
    ...
    'parallel-keyword': [definition_12],
    'function': [
